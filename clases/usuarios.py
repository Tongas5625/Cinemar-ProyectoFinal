...
#        return self.__edad >= 18
    ...
    def grabar_datos(self):
        conexion=sqlite3.connect("user.db")
        cursor=conexion.cursor()
        cursor.execute(f"INSERT INTO usuarios (Nombre, Apellido, Dni, Mail, Passwd, EsAdmin) VALUES ('{self.__nombre}','{self.__apellido}',{self.__dni},'{self.__mail}','{self.__passw}','{self.__esadmin}')")
        conexion.commit()
        conexion.close()

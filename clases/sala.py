...
class Sala:
    ...
    def base_de_datos(self):
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS salas (id INTEGER, Numero	INTEGER,Tipo TEXT, Asientos	INTEGER, PRIMARY KEY(id AUTOINCREMENT));")
        conexion.commit()
        conexion.close()
    ...
    def grabar_datos(self):  # inserta datos
        ...
        cursor = conexion.cursor()
        print(
            f"INSERT INTO salas (Numero, Tipo, Asientos) VALUES ('{self._numero}','{self._tipo}','{self._asientos}');")
        cursor.execute(
            f"INSERT INTO salas (Numero, Tipo, Asientos) VALUES ('{self._numero}','{self._tipo}','{self._asientos}');")
    ...
    def get_id_db(self):
        ...
        cursor = conexion.cursor()
        cursor.execute(
            f"SELECT id FROM salas WHERE Numero='{self._numero}' AND Tipo='{self._tipo}' AND Asientos={self._asientos};")
        mem = cursor.fetchone()
        self._id = mem[0]
        conexion.close()
        print(mem[0])
    ...
    def modificar(self):
        ...
        cursor = conexion.cursor()
        print(
            f"UPDATE salas SET Numero={self._numero}, Tipo='{self._tipo}', Asientos={self._asientos}, WHERE id={self._id};")
        cursor.execute(
            f"UPDATE salas SET Numero={self._numero}, Tipo='{self._tipo}', Asientos={self._asientos} WHERE id={self._id};")
        conexion.close()

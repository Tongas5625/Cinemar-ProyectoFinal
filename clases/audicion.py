...
class Audicion:
    ...
    def base_de_datos(self):
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS audiciones (id	INTEGER, Pelicula	INTEGER,Sala INTEGER, Fecha TEXT,Hora TEXT,PRIMARY KEY(id AUTOINCREMENT));")
        conexion.commit()
        conexion.close()
    ...
    def grabar_datos(self):  # inserta datos
        ...
        cursor = conexion.cursor()
        print(
            f"INSERT INTO audiciones (Pelicula, Sala, Fecha, Hora) VALUES ({self._pelicula},{self._sala},'{self._fecha}','{self._hora}');")
        cursor.execute(
            f"INSERT INTO audiciones (Pelicula, Sala, Fecha, Hora) VALUES ({self._pelicula},{self._sala},'{self._fecha}','{self._hora}');")
    ...
    def get_id_db(self):
        ...
        cursor = conexion.cursor()
        cursor.execute(
            f"SELECT id FROM audiciones WHERE Pelicula={self._pelicula} AND Fecha='{self._fecha}' AND Hora='{self._hora}' AND Sala={self._sala};")
        mem = cursor.fetchone()
        self._id = mem[0]
        conexion.close()
        print(mem[0])
    ...
    def modificar(self):
        ...
        cursor = conexion.cursor()
        cursor.execute(
            f"UPDATE audiciones SET Pelicula='{self._pelicula}', Fecha='{self._fecha}', Hora={self._hora}, Sala={self._sala} WHERE id={self._id};")
        conexion.commit()
        conexion.close

...
class Pelicula:
    ...
    def base_de_datos(self):
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS pelis (id	INTEGER, Nombre	TEXT, Director	TEXT, Duracion	INTEGER, Estreno	TEXT, Clasificacion	TEXT, PRIMARY KEY(id AUTOINCREMENT))")
        conexion.commit()
        conexion.close()
    ...
    def grabar_datos(self):
        ...
        cursor = conexion.cursor()
        print(
            f"INSERT INTO pelis (Nombre, Director, Duracion, Estreno, Clasificacion) VALUES ('{self._nombre}','{self._director}','{self._duracion}','{self._fechaEstreno}','{self._clasificacion}');")
        cursor.execute(
            f"INSERT INTO pelis (Nombre, Director, Duracion, Estreno, Clasificacion) VALUES ('{self._nombre}','{self._director}','{self._duracion}','{self._fechaEstreno}','{self._clasificacion}');")
    ...
    def get_id_db(self):
        ...
        cursor = conexion.cursor()
        cursor.execute(
            f"SELECT id FROM pelis WHERE Nombre='{self._nombre}' AND Director='{self._director}' AND Duracion={self._duracion} AND Estreno='{self._fechaEstreno}';")
        mem = cursor.fetchone()
        self._id = mem[0]
        conexion.close()
        print(mem[0])

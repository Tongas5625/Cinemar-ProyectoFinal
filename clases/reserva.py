...
class Reserva:
    ...
    def base_de_datos(self):
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS reservas (id	INTEGER, Butaca	INTEGER,Audicion INTEGER, Entrada INTEGER,Usuario TEXT,PRIMARY KEY(id AUTOINCREMENT));")
        conexion.commit()
        conexion.close()
    ...
    def get_id_db(self):
        ...
        conexion=sqlite3.connect("reservas.db")
        cursor=conexion.cursor()
        cursor.execute(f"SELECT id FROM reservas WHERE Butaca={self._butaca} AND Audicion={self._audicion} AND Entrada={self._entradas} AND Usuario='{self._usuario}';")
        mem=cursor.fetchone()
        self._id=mem[0]
        conexion.close()
        print (mem[0]) 
